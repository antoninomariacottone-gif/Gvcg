...
const CoinDropApp = () => {
  ...
  const initApp = async () => {
    ...
    try {
      ...
      if (error && error.code === 'PGRST116') {
        ...
        setCurrentUser(user);
        setIsAdmin(telegramUser.id === 79820559);
      }
    } catch (err) {
          ...
          return (
            <div key={task.id} className="bg-white rounded-xl shadow-md p-4">
              <div className="flex items-start gap-4">
                ...
                <button onClick={() => { if (task.link) window.open(task.link, '_blank'); completeTask(task.id); }} disabled={completed && !task.repeatable} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-semibold disabled:opacity-50 hover:bg-blue-700 whitespace-nowrap">
  ...
  const AdminDashboard = () => {
    ...
    useEffect(() => {
      const load = async () => {
        ...
        setStats({ users: users.length, totalCoins: total, pending: withdrawals.length, rate });
      };
      load();
    }, []);
  ...
  const AdminUsers = () => {
    ...
    const adjust = async (add) => {
      if (!selected || !amount) return;
      const newBalance = Math.max(0, selected.balance + (add ? parseFloat(amount) : -parseFloat(amount)));
      await supabase.from('users').update({ balance: newBalance }).eq('id', selected.id);
    ...
    return (
      <div className="p-6">
        ...
        <div className="mb-4 relative">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cerca..." className="w-full pl-10 pr-4 py-3 border rounded-lg" />
        </div>
        <div className="space-y-3 mb-24">
          {filtered.map(u => (
            <div key={u.id} onClick={() => setSelected(u)} className={`bg-white rounded-xl shadow p-4 cursor-pointer ${selected?.id === u.id ? 'ring-2 ring-blue-500' : ''}`}>
              <div className="flex justify-between">
                <div>
                  <div className="font-bold">@{u.username}</div>
                  <div className="text-sm text-gray-600">ID: {u.id}</div>
                  <div className="text-sm text-green-600">Referral Earnings: {u.referral_earnings}</div>
                </div>
                <div className="text-right">
                  <div className="text-2xl font-bold text-blue-600">{u.balance.toLocaleString()}</div>
        ...
        {selected && (
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg p-4">
            <h3 className="font-bold mb-3">Modifica: @{selected.username}</h3>
            <div className="flex gap-2">
              <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="Importo" className="flex-1 border rounded-lg p-2" />
              <button onClick={() => adjust(true)} className="bg-green-600 text-white px-4 rounded-lg"><Plus className="w-5 h-5" /></button>
              <button onClick={() => adjust(false)} className="bg-red-600 text-white px-4 rounded-lg"><Minus className="w-5 h-5" /></button>
            </div>
          </div>
        )}
      </div>
  ...
  const AdminTasks = () => {
    ...
    const save = async () => {
      ...
      await supabase.from('tasks').insert([{...form, reward: parseInt(form.reward), active: true}]);
    ...
    return (
      <div className="p-6">
        ...
        <div className="bg-white rounded-xl shadow p-6 mb-6">
          <h3 className="font-bold mb-4">Crea Task</h3>
          <div className="space-y-3">
            <input type="text" value={form.title} onChange={(e) => setForm({...form, title: e.target.value})} placeholder="Titolo" className="w-full border rounded-lg p-2" />
            <input type="text" value={form.description} onChange={(e) => setForm({...form, description: e.target.value})} placeholder="Descrizione" className="w-full border rounded-lg p-2" />
            <input type="text" value={form.link} onChange={(e) => setForm({...form, link: e.target.value})} placeholder="Link (opzionale)" className="w-full border rounded-lg p-2" />
            <input type="number" value={form.reward} onChange={(e) => setForm({...form, reward: e.target.value})} placeholder="Ricompensa CDC" className="w-full border rounded-lg p-2" />
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={form.repeatable} onChange={(e) => setForm({...form, repeatable: e.target.checked})} className="w-4 h-4" />
              <span className="text-sm">Ripetibile</span>
            </label>
            <button onClick={save} className="w-full bg-blue-600 text-white rounded-lg py-2 font-semibold">Crea</button>
          </div>
        </div>
        <div className="space-y-3">
          {tasks.map(t => (
            <div key={t.id} className="bg-white rounded-xl shadow p-4">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="text-sm text-green-600 mt-1">+{t.reward} CDC</div>
                </div>
                <div className="flex gap-2">
                  <button onClick={() => toggle(t)} className={`px-3 py-1 rounded-lg text-sm ${t.active ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'}`}>
                    {t.active ? 'Attivo' : 'Off'}
                  </button>
                  <button onClick={() => remove(t.id)} className="p-2 bg-red-100 text-red-600 rounded-lg">
  ...
  const AdminWithdrawals = () => {
    ...
    const reject = async (w) => {
      ...
      await supabase.from('users').update({ balance: user.balance + w.cdc_amount }).eq('id', w.user_id);
      const { data } = await supabase.from('withdrawals').select('*').order('date', { ascending: false });
      setWithdrawals(data);
    };
    ...
    return (
      <div className="p-6">
        <h2 className="text-2xl font-bold mb-4">Prelievi</h2>
        <div className="space-y-3">
          {withdrawals.map(w => (
            <div key={w.id} className="bg-white rounded-xl shadow p-4">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="text-sm text-gray-600">${w.usdt_amount} USDT ({w.cdc_amount} CDC)</div>
                ...
                <div className="flex items-center gap-2">
                  {w.status === 'pending' && (
                    <>
                      <button onClick={() => approve(w)} className="bg-green-500 text-white p-2 rounded-lg">
                        <CheckCircle className="w-5 h-5" />
                      </button>
                      <button onClick={() => reject(w)} className="bg-red-500 text-white p-2 rounded-lg">
                        <XCircle className="w-5 h-5" />
                      </button>
                    </>
                  )}
                  {w.status === 'approved' && <span className="text-green-600 text-sm font-semibold">✓ Approvato</span>}
                  {w.status === 'rejected' && <span className="text-red-600 text-sm font-semibold">✗ Rifiutato</span>}
                </div>
              </div>
            </div>
          ))}
  ...
  const AdminSettings = () => {
    ...
    useEffect(() => {
      const load = async () => {
        const { data } = await supabase.from('system').select('total_funds').eq('id', 1).single();
    ...
    const update = async () => {
      await supabase.from('system').update({ total_funds: parseFloat(funds) }).eq('id', 1);
    ...
    return (
      <div className="p-6">
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <input type="number" step="0.01" value={funds} onChange={(e) => setFunds(e.target.value)} className="w-full border rounded-lg p-3 mb-4" />
          <button onClick={update} className="w-full bg-blue-600 text-white rounded-lg py-3 font-bold">Aggiorna</button>
        </div>
      </div>
    );
  };
        ...
        <div className="bg-white rounded-2xl p-8 max-w-md w-full">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={referralCode} onChange={(e) => setReferralCode(e.target.value.toUpperCase())} placeholder="REF123456" className="w-full border-2 rounded-lg p-3 text-center text-lg font-mono tracking-wider uppercase" />
      ...
      <div className="min-h-screen bg-gray-100 pb-20">
        ...
        <div className="flex border-b bg-white overflow-x-auto">
          {[
            ...
            <button key={tab.id} onClick={() => setAdminTab(tab.id)} className={`flex items-center gap-2 px-6 py-4 font-semibold whitespace-nowrap ${adminTab === tab.id ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-600'}`}>
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-4">
        <div className="flex justify-between items-center">
          ...
          {currentUser.id === 79820559 && (
            <button onClick={() => setIsAdmin(true)} className="bg-white/20 p-2 rounded-lg">
              <Shield className="w-6 h-6" />
            </button>
          )}
        </div>
